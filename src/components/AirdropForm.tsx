...
async function getApprovedAmount(tSenderAddress: string | null): Promise<number> {
  ...
  if (!tokenAddress || !account.address) {
    alert("Missing token address or wallet not connected");
    return 0;
  }
  ...
  try {
    ...
    // readContract returns `bigint` by default
    return Number(response); // Convert bigint to number
  } catch (error) {
    console.error("Failed to read allowance:", error);
    alert("Something went wrong while reading allowance.");
    return 0;
  }
}
